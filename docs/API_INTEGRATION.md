...
`POST /api/meals/analyze` accepts `file` plus a manual `weight_grams`. A `200` response can have `calculated`, `food_not_recognized`, `requires_food_selection`, or `nutrition_reference_not_found`. `requires_food_selection` means Nutri-Box did not divide a shared plate weight among foods; obtain or confirm an individual portion before creating a meal. When local/alias resolution misses, USDA candidates are deterministically filtered and ranked for relevance before this existing selection outcome. `recognized_foods` always contains the original normalized recognition output, not USDA display descriptions. For authenticated analysis-session responses, candidate identity and display data are in `components[].candidates`; clients must select a candidate identifier rather than infer identity from a display name.

Paired NutriBox Pi devices may use the same analysis, analysis-session continuation/selection, and completed-session meal-save routes with `X-Device-Token` instead of a user bearer token. The backend derives the paired owner and creates/logs data under that owner; it never returns or reconstructs the user token. Device requests may save only `analysis_session_id` meals, not manual item payloads. Existing unauthenticated analysis remains available, bearer behavior is unchanged, and a request containing both bearer and device credentials is rejected with `400`. `GET /api/device/me` returns device metadata plus only `owner_first_name` for the Pi greeting.
