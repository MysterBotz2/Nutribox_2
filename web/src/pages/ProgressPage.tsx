...
import { weightEntriesApi } from '../api/weight-entries'
...
export function ProgressPage() {
  ...
  const weightEntries = useQuery({ queryKey: queryKeys.weightEntries, queryFn: weightEntriesApi.list })
  return <div className="page-stack"><header className="page-header"><p className="eyebrow">Progress</p><h1>Nutrition progress</h1><p className="muted">Backend-authoritative analytics · {timezone}</p></header>
    ...
    <section className="card"><h2>Weight history</h2><p className="muted">Chronological entries recorded in your account.</p>{weightEntries.isPending ? <StateMessage>Loading weight history…</StateMessage> : weightEntries.isError || !Array.isArray(weightEntries.data?.entries) ? <StateMessage kind="error">Unable to load weight history.</StateMessage> : <WeightHistoryChart {...weightEntries.data} />}</section>
  </div>
}
